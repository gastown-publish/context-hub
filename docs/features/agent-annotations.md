...
1. **Private annotations** — stored locally in `~/.gashub/annotations/`, automatically included when the agent fetches the same doc/skill again. Personal knowledge base that improves with use.
...
Stored as markdown files at `~/.gashub/annotations/<id>.md` (e.g. `~/.gashub/annotations/openai/chat.md`):
...
### `gashub annotate <id> <message>`

Add an annotation to a doc or skill.

```bash
# Agent adds a gotcha after encountering an issue
gashub annotate openai/chat "streaming requires explicit close when using function calling"

# Add with a type prefix
gashub annotate openai/chat --type gotcha "streaming requires explicit close..."
gashub annotate openai/chat --type tip "batch function calls for parallel execution"
```

- Creates `~/.gashub/annotations/openai/chat.md` if it doesn't exist
- Appends a new `## Gotcha:` or `## Tip:` section
- Updates the `updated` timestamp in frontmatter

### `gashub get <id>` — automatically includes annotations
...
```bash
gashub get openai/chat
# Returns: doc content + "# Annotations\n## Gotcha: streaming requires..."

gashub get openai/chat --no-annotations
# Returns: doc content only
```

### `gashub annotations list`
...
### `gashub annotations show <id>`

Show annotations for a specific doc/skill.

### `gashub annotations clear <id>`
...
- `getAnnotationPath(id)` — returns `~/.gashub/annotations/<id>.md`
...
- `gashub annotate <id> <message>` command
- `gashub annotations list|show|clear` subcommands
...
1. `gashub annotate openai/chat "streaming needs explicit close"` — creates annotation file
2. `gashub annotate openai/chat --type tip "batch function calls"` — appends to file
3. `gashub annotations list` — shows openai/chat with 2 annotations
4. `gashub annotations show openai/chat` — shows both annotations
5. `gashub get openai/chat` — doc content + annotations appended
6. `gashub get openai/chat --no-annotations` — doc content only
7. `gashub annotations clear openai/chat` — removes annotation file
